...
**[2026-07-25, Timothy]** Wrote `.github/workflows/docker_image_release.yml` directly, ahead of Stage 3 — triggers on push to `main` (branch push) and on `v*.*.*` tags, with `latest` enabled only on the `main` branch and a `type=semver` tag enabled only on an actual version tag. Answers the publish-trigger question above: both, exactly as guessed as the working default.

**[2026-07-25, AI]** Reviewed against the actual file. The trigger/tag logic is right and matches the intent exactly — `enable=${{ github.ref == 'refs/heads/main' }}` correctly scopes `latest` to branch pushes, and `type=semver,pattern={{version}}` only produces a tag (a bare `1.0.0`, `v` stripped) when the ref is an actual version tag, never on a `main` push. Two fixes made directly in the file, not just noted:

1. **Real bug, would have failed on the first run:** `images: ghcr.io/${{ github.repository }}` used the repo's name as-is — `Timothy-Lee-Grant/Tool_Box`, mixed case — but GHCR (like every OCI registry) rejects uppercase in image references outright (`invalid reference format: repository name must be lowercase`). Added a step that lowercases it into `$GITHUB_ENV` before `docker/metadata-action` consumes it, so the published image resolves to `ghcr.io/timothy-lee-grant/tool_box` — matching the lowercase path already assumed in this doc's own Packaging Plan example (`ghcr.io/timothy-lee-grant/tool_box:1.0.0`).
2. **Missing prerequisite:** no `docker/setup-buildx-action` step. `docker/build-push-action` expects a Buildx builder to already exist — every one of Docker's own examples includes this as a required setup step before the build step, so it was added.

Two things left as manual, one-time operational steps — not code, so not part of the workflow file itself: (a) **GHCR packages default to private on first publish** — after the first successful run, Timothy needs to open the package's own Settings on GitHub and set visibility to public, or LLM_Monitor's `docker pull` will fail with a 401/403 despite the workflow succeeding; (b) if the very first push fails with a permissions error despite `packages: write` being set in the workflow, check **Settings → Actions → General → Workflow permissions** on the repo — it must allow read/write, not be locked to read-only at the org/repo level.

Net effect: the GHCR publish job the Release Checklist called for now exists in the repo, reviewed and fixed. What remains before this thread of Stage 3 is fully closeable: a first real tag push (`v1.0.0`) to prove the whole pipeline end to end, then the one-line `docker-compose.yml` change on the LLM_Monitor side (§Q4 above).

Stage 3 (step-by-step implementation plan) is not drafted yet — pending Timothy's direction on the remaining open items above (LICENSE, version-vs-plan-004 sequencing, presentation timing).

---

**[2026-07-26, Timothy, via the LLM_Monitor session]** `./build.sh --mode live` on the consuming project failed: `toolbox Pulling ... no matching manifest for linux/arm64/v8 in the manifest list entries`. Timothy's own machine is Apple Silicon. Confirmed the package is public (that wasn't the issue) and was explicit he wants the registry integration kept — not reverted to `build: context: ../Tool_Box` — because the point is professional cross-project integration, not pulling from his own machine.

**[2026-07-26, AI, from the LLM_Monitor session]** Root-caused against the actual files, not assumption: `docker manifest inspect ghcr.io/timothy-lee-grant/tool_box:1.0.0` shows exactly one platform, `amd64`. This is a gap in the Stage-3 review two entries above — that pass caught the lowercase-image-name bug and the missing Buildx setup step, but missed that `docker/build-push-action` without an explicit `platforms:` key only builds for the *runner's own* platform (`ubuntu-latest` = amd64), full stop — there's no implicit multi-arch behavior to opt out of. Worth naming plainly since I did that review: the check wasn't broad enough at the time.

**Fixed directly in `docker_image_release.yml`** (same file, same pattern as that earlier pass — this workflow apparently gets fixed in place rather than argued about first, matching how the lowercase/buildx issues were handled):
1. Added `docker/setup-qemu-action@v3` before the Buildx setup step — provides the emulation binaries the amd64 runner needs to cross-build for arm64.
2. Added `platforms: linux/amd64,linux/arm64` to the `build-push-action` step — this is what actually turns "build once" into "build once per platform, publish one multi-arch manifest list."

Why this should work cleanly (not just "adding the standard flags and hoping"): the Dockerfile's build stage runs a plain `dotnet publish` with no `-r <RID>` — a framework-dependent, portable-IL output, not a self-contained/AOT native build. That means there's no architecture-specific compilation output to get wrong; the same published DLLs run under whichever architecture's `mcr.microsoft.com/dotnet/aspnet:10.0` the runtime stage resolves to (Microsoft's own images are already multi-arch). QEMU only has to emulate the SDK long enough to run `dotnet restore`/`dotnet publish` for the arm64 leg, not cross-compile native code — the slower, riskier version of this problem doesn't apply here.

**What's still a manual step, not something a workflow file can do:** the already-published `1.0.0` tag is the amd64-only image forever — GHCR tags aren't mutated by re-running an old workflow. A new tag push (or a `main` push, if `latest` is what LLM_Monitor should track instead) is required to produce the multi-arch manifest under a version this fix actually applies to. That's Timothy's call — same open question as "Versioning" in Stage 2 above (does 1.0.0 get re-cut, or does this land as 1.0.1) — not decided here. Once a new tag is pushed and confirmed multi-arch (`docker manifest inspect` should show both `amd64` and `arm64` platform entries), the fix on the LLM_Monitor side is just updating `docker-compose.yaml`'s image tag to match.

---

**[2026-07-26, Timothy]** Tool calls work over the compose network (confirmed in the arm64 session above), but asked to figure out how to actually *see* the world in a browser once Tool_Box runs inside LLM_Monitor's Docker setup, rather than directly on a laptop via stdio.

**[2026-07-26, AI]** Investigated both repos. The viewer (`viewer/index.html` + `VoxelViewerBroadcastService`, ADR-010) is a second, deliberately independent channel from MCP — the agent never touches it — and it had never actually worked through any Docker path, in either repo, since plan 003 shipped it. Two stacked problems, both real:

1. **The listener bound loopback-only.** `VoxelViewerBroadcastService.StartListener()` used `listener.Prefixes.Add($"http://127.0.0.1:{port}/voxel/")`. Inside a container, `127.0.0.1` is the container's own private network namespace — Docker's `-p hostPort:containerPort` forwards traffic addressed to the container's real interface, which a loopback-only socket never sees. No `ports:` mapping in any compose file, however configured, could have fixed this; the bind itself had to change.
2. **Nothing published the port.** Neither Tool_Box's own dev compose nor LLM_Monitor's consuming compose exposed 8090-8093 — only the MCP port (8081→8080 / unpublished, respectively) was ever mapped.

**Fixed, in order, and documented as ADR-012** (`docs/DECISIONS.md`) rather than left as an undocumented patch, since this is a real architecture decision (a socket-binding fix *and* a conscious exposure call), the same pattern this project uses for ADR-008/009/010/011:

1. `VoxelViewerBroadcastService.cs` — bind changed to `http://+:{port}/voxel/` (HttpListener's strong wildcard, all interfaces) — the same class of fix `ToolBoxHttpApp.cs` already needed for the MCP endpoint (`ASPNETCORE_URLS=http://0.0.0.0:8080`), just expressed in `HttpListener`'s own prefix syntax. `+` still accepts `127.0.0.1` connections, so the direct-on-host/stdio scenario this toolset originally shipped for is unaffected. Log message updated to stop asserting a specific host now that it binds broadly.
2. `Dockerfile` — added `EXPOSE 8090` alongside the existing `EXPOSE 8080` (documentation only; doesn't publish anything by itself, but matches the existing convention and signals the port exists).
3. Tool_Box's own `docker-compose.yml` — added `"8090:8090"` to the `toolbox` service's existing `ports:` list, so the standalone dev compose (which this repo's own Release Checklist already requires re-verifying end to end) can prove the viewer works through a container, not just through `dotnet run`.
4. `docs/TOOL_CATALOG.md` and `docs/LLM_MONITOR_INTEGRATION.md` — the catalog's Voxel notes no longer assert `127.0.0.1` as *the* bind address (now: reachable at `localhost:8090` directly, or at whatever host a container publishes 8090 to); the integration doc gained an explicit, opt-in `ports: - "8090:8090"` example, framed as a decision separate from the MCP endpoint's continued lockdown.
5. **LLM_Monitor's `docker-compose.yaml`** — added the same `"8090:8090"` mapping to its `toolbox` service, with a comment explaining why this doesn't reopen ADR-008/011's lockdown: the viewer channel is receive-only/human-eyes-only by construction, so publishing it grants a caller zero tool-execution capability, unlike the MCP port (8080), which stays unpublished exactly as before.

**Why exposing 8090 is not a security regression, stated explicitly (the thing ADR-011 already modeled — say the reasoning out loud, don't just do it):** ADR-008/011's posture is about the *MCP endpoint* — the channel that lets a caller execute tools, including write-classified ones. The viewer WebSocket has no request/command surface at all; `VoxelViewerBroadcastService` only ever sends snapshot/diff messages outward and reads just enough to detect a close frame (§17 of Learning doc 004). Publishing it lets a browser watch, and nothing else.

**What doesn't take effect yet:** the fix lives in source and in Tool_Box's own compose/Dockerfile now, but LLM_Monitor's pinned tag (`ghcr.io/timothy-lee-grant/tool_box:1.0.1`) predates it — the `ports:` entry just added there is inert until a new tag is built and pushed and that pin is bumped, per Timothy's request not to handle tagging as part of this fix. Verification once that happens: `docker compose up`, then open `viewer/index.html` in a browser (it hardcodes `ws://127.0.0.1:8090/voxel/`, which resolves correctly once the port is published to the host), call a `place_*` tool through LLM_Monitor's agent loop, confirm the build appears live.

---

### [2026-07-26] Verified live — and the real remaining blocker was neither the bind nor the port mapping

Timothy switched LLM_Monitor's `toolbox` service to `build: context: ../Tool_Box` specifically so this fix could be verified locally before committing to another tag/publish cycle (correct call — no point re-tagging until proven). Visiting `http://localhost:8090/` in a browser returned `Not Found`. Investigated end to end rather than assumed:

- `curl http://localhost:8090/` → real `404` from `Server: Microsoft-NetCore/2.0` (expected — `/` was never a registered `HttpListener` prefix, only `/voxel/` is; this response, on its own, is not a bug).
- A raw WebSocket client connected to `ws://127.0.0.1:8090/voxel/` successfully and received a `snapshot` message — but always `{"type":"snapshot","blocks":[]}`, even immediately after placing real blocks through LLM_Monitor's live agent loop and confirming them via `describe_world` (which correctly reported the true count every time).
- Traced it with a temporary diagnostic (hash-code logging on both the MCP tool path and the broadcast service's `VoxelWorld` reference, since inline `[diag]` logs revert cleanly and this repo's own convention already favors verifying against real behavior over assumption): **both resolved the exact same object** (`GetHashCode() == 31364015` on both sides), and the broadcast service's `Changed` event fired correctly on every mutation — to `sockets=0`. The DI wiring, the singleton registration, the event subscription: all correct, exactly as written.
- Root cause was outside the code entirely: `lsof -nP -iTCP:8090` on the host showed a **second, native `ToolBox.Host` process** (`/Users/timothygrant/Desktop/projects/Tool_Box/src/ToolBox.Host/bin/Release/net10.0/ToolBox.Host`, PID 80681, running since **2026-07-22 23:18**, days before this thread started) bound to `127.0.0.1:8090` — almost certainly a leftover from earlier local Claude Desktop/Code stdio testing. Because Voxel's viewer service runs identically under any transport (ADR-010), that host-native process had *also* started its own `VoxelViewerBroadcastService`, and got to port 8090 first. Docker's own port-publishing proxy could only bind the wildcard address (`*:8090`); macOS routes connections addressed to the literal `127.0.0.1` (which is what `viewer/index.html`'s hardcoded `WS_URL` connects to) to the more specific listener — the native process — ahead of Docker's wildcard one. Every test connection was silently landing on an orphaned, always-empty `VoxelWorld` that nothing had ever mutated, while the real Docker container's world sat there correctly updated and correctly broadcasting to zero connections.
- **Fix: `kill 80681`.** Not a code change — the "+" bind fix and the port-publish decision above were both already correct and necessary; this was a pre-existing, unrelated local process squatting on the same port. Rebuilt `toolbox` clean (diagnostics reverted, diff-verified against the ADR-012 commit), reconnected the viewer, and confirmed live end-to-end: placed a 3×3×3 gold cube via a real Groq call through LLM_Monitor's `graph-free` pipeline, watched it render in the browser in real time (`BLOCKS 27`, `BOUNDS 3×3×3`, status `LIVE`).
- **Worth a note for the Release Checklist's "re-run the Claude Desktop/Code quickstart on a clean config" item above:** a stdio session left running in the background is exactly the kind of stale local state that item is meant to catch — this is a concrete instance of it, not a hypothetical.
