...
*(Still true for the local/demo path described above — but not the whole story once LLM_Monitor integration became real: the viewer's listener originally bound loopback-only, which meant it could never work through a container regardless of "orthogonal" intent. Release 1.0's ADR-012 revisited this once it actually mattered — `docs/DECISIONS.md`.)*
