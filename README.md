...
| **Engineering judgment under review** | Every non-trivial decision is a dated, append-only [Architecture Decision Record](docs/DECISIONS.md) (12 so far) — including one that explicitly revises an earlier security assumption once circumstances changed, with the reasoning written down rather than silently overridden. |
...
<p align="center">
  <img src="docs/images/voxel_world.gif" alt="The world in motion, captured live from the browser viewer as tool calls land" width="850">
</p>
...
- **Plan 004** — A SPICE-backed electrical circuit design toolset (describe a circuit, get it simulated and exported). *Reopened, pre-implementation.* A concepts/feasibility pass ([Lecture 008](Documentation/Learning/008-The-Solver-And-The-Draftsman.md), verified against a from-scratch MNA solver in `Documentation/Learning/008-Spikes/`) is done and fed back into the plan; Stage 3's step-by-step build plan is not drafted yet — it waits on an actual `ngspice` round-trip spike (blocked so far by no package-install access in the environment this work was done in).
- **Plan 005 (Release 1.0)** — *Done.* `v1.0.0`, `v1.0.1`, `v1.0.2` tagged and published to GHCR (multi-arch, `linux/amd64`+`linux/arm64`); real cross-project consumption verified end to end — LLM_Monitor's LangGraph agent calling Voxel tools live through a real model, rendered in the browser viewer through the container (ADR-012). LICENSE decision remains open.
- **Post-release** — [Lecture 009](Documentation/Learning/009-The-Whole-Machine.md): a capstone teaching document plus an adversarial audit of the shipped 1.0, including a genuine data-race finding in `VoxelWorld`'s singleton state.
- **Next** — resolve the LICENSE decision; run the `ngspice` spike and resume plan 004.
